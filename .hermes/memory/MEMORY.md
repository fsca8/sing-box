...
§
配置双文件: profiles.json的config=JSON字符串(非base64),启动时覆盖sing-box-config.json→改MTU等须改profiles.json; Windows更新UAC进程普通taskkill杀不掉→powershell Start-Process taskkill -Verb RunAs,备份singbird-fix/old-win-backup再替换; adb pull须C:/绝对
§
...
§
netbird自定义域名实时注入(8-15): route.rule_set注入本地rule-set(DataDir/nb-domains.json须带version)+route/dns规则引用; fswatch重写即运行时生效(原地写,rename断inotify); 文件须先于配置加载存在